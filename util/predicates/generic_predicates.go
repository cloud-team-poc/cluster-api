...
// ResourceNotExternallyManaged returns a predcate that returns true only if the resource does not contain
...
func processIfNotExternallyManaged(logger logr.Logger, obj client.Object) bool {
	kind := strings.ToLower(obj.GetObjectKind().GroupVersionKind().Kind)
	log := logger.WithValues("namespace", obj.GetNamespace(), kind, obj.GetName())
	if annotations.HasExternallyManagedAnnotation(obj) {
